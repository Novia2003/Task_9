...
public class FrameMain extends JFrame {
    ...
    public FrameMain() {
        ...
        buttonSaveToInputFile.addActionListener(actionEvent -> {
            try {
                if (fileChooserSave.showSaveDialog(panelMain) == JFileChooser.APPROVE_OPTION) {
                    int[] firstArray = JTableUtils.readIntArrayFromJTable(tableFirstList);
                    int[] secondArray = JTableUtils.readIntArrayFromJTable(tableSecondList);
                    String file = fileChooserSave.getSelectedFile().getPath();
                    if (!file.toLowerCase().endsWith(".txt")) {
                        file += ".txt";
                    }
                    ArrayUtils.writeArrayToFile(file, firstArray);
                    ArrayUtils.writeArrayToFile(file, secondArray);
                }
            } catch (Exception e) {
        ...
        buttonCreateNewList.addActionListener(actionEvent -> {
            try {
                List<Integer> firstList = ArrayUtils.intArrayToList(Objects.requireNonNull(JTableUtils.readIntArrayFromJTable(tableFirstList)));
                List<Integer> secondList = ArrayUtils.intArrayToList(Objects.requireNonNull(JTableUtils.readIntArrayFromJTable(tableSecondList)));
